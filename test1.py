...
class SmaCrossStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:  # not in the market
            ...
            if self.position.size > 0 and self.dataclose[0] < self.sma_fast[0]:
                self.log('SELL CREATE, %.2f' % self.dataclose[0])
                self.order = self.sell() # close long
            elif self.position.size < 0 and self.dataclose[0] > self.sma_fast[0]:
                self.log('BUY CREATE, %.2f' % self.dataclose[0])
                self.order = self.buy()  # close short
